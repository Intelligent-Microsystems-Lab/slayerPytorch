...
class Network(torch.nn.Module):
    ...
    def forward(self, spikeInput):
        ...
        self.threshold = np.percentile(distr, args.threshold)
        dropped_frames_counter = 0
        for batch in range(4):
            for timestep in range(1450):
                frameDensity = np.count_nonzero(spike[batch,:,:,:,timestep].cpu().detach().numpy())
                if frameDensity < self.threshold:
                    frame_counter+=1
                else:
                    frame_counter = 0
                if frame_counter == args.frame_drope:
                    #spike[batch,:,:,:,timestep] = spike[batch,:,:,:,timestep-1] = spike[batch,:,:,:,timestep-2] = 0
        ...
        # counter = 0         
        # for timestep in range(1450):
        #     frameDensity = np.count_nonzero(spike[0,:,:,:,timestep].cpu().detach().numpy())
        #     distr.append(frameDensity)
        #     time.append(timestep)
        # threshold = np.percentile(distr, 25)
        # tripletCounter = 0
        # for batch in range(4):
        #     for timestep in range(1450):
        #         frameDensity = np.count_nonzero(spike[batch,:,:,:,timestep].cpu().detach().numpy())
        #         if frameDensity < threshold:
        #             tripletCounter+=1
        #         else:
        #             tripletCounter = 0
        #         if tripletCounter == 3:
...
if __name__ == '__main__':
    ...
    for epoch in range(1):
        ...
        filename=str('validate/dropout_' + str(args.droprate))
...
#   plt.legend()
#   plt.show()
    print( 'Accuracy with ' + str(args.frame_drope) + ' consec frame droppe:  ' + str(stats.testing.accuracyLog))
    ...
    header = ['window' ,'dropped_frame' ,'accuracy' ,'threshold']
    with open('validate/drop.csv', 'a', encoding='UTF8') as f:
        writer = csv.writer(f)
